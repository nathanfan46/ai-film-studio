...
**`generate-motion-transfer` is a separate, human-directed alternative to `generate-video`, not something you reach for on your own.** It retargets a driving reference video's motion onto the shot's locked character reference image instead of generating from the prompt — useful for precise, counted choreography a text description can't reliably reproduce. It only applies when the shot's `driving_video.path` is already set, which only happens when the human has explicitly supplied (or asked for) a driving clip for that specific shot. Never set `driving_video.path` or switch a shot to `generate-motion-transfer` on your own initiative; if a shot has no `driving_video.path`, keep using `generate-video` as above.
