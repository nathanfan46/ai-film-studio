...
Do this before anything else. Do not require the user to manually activate a venv if you can avoid it. Only ever use one of the two fixed forms below (never a machine-specific absolute path) — this repo ships `.claude/settings.json` pre-authorizing the non-spend `ai-film` subcommands under these two forms (`init`, `version`, `validate`, `status`, `models`, `check-continuity`, `review`, `select-candidate`), so a working run shouldn't need Bash permission prompts for those. Run each check as a single, plain command — never chain it with `; echo ...` or any other trailing command to inspect the exit code; your Bash tool already reports success/failure and any error output directly in its own result.
