...
if kind == "character":
    ...
    if len(parts) == 4 and parts[2] == "turnaround":
        _validate_angle_segment(parts[3])  # non-empty, no /, \, or .. — see below
...
**Open-ended does not mean unvalidated.** `orientation` (and the `<angle>` segment of a turnaround target) ends up as a literal filesystem path component in both `target_dir` and the resolver — an unvalidated value like `"../evil"` or `"side/../../etc"` would do exactly the kind of path-escape this project has already had to guard against for template and workflow ids. Both the turnaround branch of `target_dir` and `resolve_character_reference` validate the angle segment the same way `template_store.py`'s `_validate_template_id`/`workflow_service.py`'s `_validate_workflow_id` already validate theirs: non-empty, and rejecting `/`, `\`, and `..` before any path join. This keeps the vocabulary genuinely open (`left_three_quarter`, `profile_left`, anything a future project wants) while never letting it become an arbitrary path.

What *is* fixed for V1 is narrower: `ai-film-character.md`'s post-lock offer only proposes three default angles (`three_quarter`, `side`, `back`) rather than prompting for an open-ended list — keeping the agent's own UX simple — and `resolve_character_reference`'s fallback logic doesn't need a translation table because Storyboard writes `orientation` using the same angle-name strings the character's turnaround directories use directly. Both of these are agent/UX conveniences, not schema constraints, and can grow independently later without touching the mechanism described here. `side` and `three_quarter` are V1's canonical views and are not guaranteed to be correct for an asymmetric character (different detail on the left vs. right — an asymmetric haircut, a scar, a holster, a one-sided tattoo) — a genuinely asymmetric character may need `left_three_quarter`/`right_three_quarter`/`left_side`/`right_side` later, which the open-ended mechanism already supports without a redesign; V1 does not attempt to detect or force this automatically.

## Generation: reference conditioning and the isolation requirement

**Every locked character image — primary and every turnaround angle — must be an isolated character reference on a plain, uniform neutral background: no environment, scenery, architectural elements, props, narrative setting, or cinematic background effects.** This is a real, go-forward change to the *existing* primary-reference prompt guidance in `ai-film-character.md` (which currently says only "matching the cinematic tone from `story.md`," with no explicit exclusion of background/scene elements) — not just new behavior for turnaround angles.

Concrete prompt requirement, appended to every character-locking prompt (primary and turnaround alike):

> "isolated character reference, plain uniform neutral background, no environment, no scenery, no architectural elements, no props, no narrative setting, no cinematic background effects, full body, consistent studio-style lighting"
...
**Every turnaround angle is independently conditioned on the locked primary reference, never on another generated turnaround angle.** Concretely: `three_quarter`, `side`, and `back` each attach `assets/characters/<name>/reference.png` directly — never `side` conditioned on `three_quarter`'s output, never `back` conditioned on `side`'s output. Chaining would let small drifts compound angle-to-angle (a jacket detail that drifts slightly at `three_quarter`, then drifts further from that already-drifted state at `side`); conditioning every angle independently on the one locked, human-approved primary bounds the drift to a single hop every time, at the cost of nothing (the primary is already the one asset guaranteed to exist and be approved before any turnaround generation starts).

`generate_candidates_cmd` currently only auto-gathers `reference_paths` for `shot:` targets; extend it so a `character:<name>:turnaround:<angle>` target auto-attaches `assets/characters/<name>/reference.png` as a reference path — always the primary, regardless of which angle is being generated or what other angles already exist — the same way a shot target auto-gathers its character/environment references. The agent still supplies `--prompt` (built from the character bible + the target angle + the isolation requirement above + an explicit "preserve exact outfit, hairstyle, proportions — do not redesign" instruction, which **reduces avoidable visual drift, and is not a consistency guarantee** — an image model can still drift a detail between angles, which is exactly why the existing human/agent candidate-review loop stays the real check, unchanged), but does not need to manually pass `--reference` since the engine already knows which reference a turnaround target implies.
...
def resolve_character_reference(project_dir: Path, character_name: str, orientation: str | None) -> str:
    if orientation:
        _validate_angle_segment(orientation)  # same guard as target_dir's turnaround branch
...
`ai-film-storyboard.md` gains one new judgment call per character per shot: alongside deciding `camera` (shot framing — `wide`/`medium`/`close-up`/etc., an independent axis, unchanged), decide `orientation`. **`orientation` identifies which character-facing view should be used as the visual identity reference for this shot — it is not the camera framing and not the camera's position relative to the character.** A shot written as the character walking away from camera implies `orientation: "back"` regardless of whether the camera itself is wide or close; an over-the-shoulder shot implies `orientation: "three_quarter"` or `"side"` depending on blocking, independent of whatever `camera` value is also set. When in doubt or when the shot doesn't call for anything unusual, omit `orientation` entirely (defaults to front via the resolver's fallback) — this is not a field Storyboard needs to fill in on every shot, only when it materially matters.
...
1. `NEEDS_INPUT`, `type: confirmation`: "Generate a turnaround set (three-quarter, side, back) for continuity across angles? This runs the candidate generation workflow for each of the three angles." (Not "3 generation rounds" — each angle's own candidate loop can itself produce several candidate images before one is locked, so "round" and "generation" aren't the same unit; the prompt should describe what will run, not a specific count of image generations it can't actually promise.) Skippable — a `HUMAN_RESPONSE` of no ends the agent's work for this character exactly as today.
2. If yes: for each of the three angles in order, run the existing generate/review/edit/select loop against `character:<name>:turnaround:<angle>`, using a prompt built from the character bible + the angle + the isolation and consistency-preservation requirements above. The human can complete some angles and skip others (e.g. lock `three_quarter` and `side` but decide `back` isn't worth it for this character) — this is a normal, supported outcome, not a partial failure: the resolver already falls through to the primary reference for any angle that was never locked, exactly as it does for a character with no turnaround set at all.
...
- `candidate_store.py`: `target_dir`/`scope_for_target` accept the new 4-part turnaround target shape; reject a malformed one (wrong segment count, wrong literal `"turnaround"` segment) with a clear `ValueError`, matching the existing malformed-target error style; reject an angle segment containing `/`, `\`, or `..`, or an empty angle segment, before any path is constructed from it.
...
- **Independent-conditioning regression test**: generating `three_quarter`, `side`, and `back` (in that order, with `three_quarter` and `side` already locked before `back` is requested) each attach `assets/characters/<name>/reference.png` as the reference path — never a previously-locked turnaround angle's file. This specifically guards against an easy-to-introduce regression where a later angle accidentally conditions on an earlier one instead of always on the primary.
- `resolve_character_reference`: returns the turnaround path when it exists; falls back to the primary reference when the angle file doesn't exist, when `orientation` is `None`, and when the character has no turnaround set at all (a project that never used this feature); rejects an unsafe orientation string the same way `target_dir`'s turnaround branch does.
...
A user locks a character, is offered (and can decline) a turnaround set, generates three-quarter/side/back candidates each independently conditioned on the same locked primary identity, and later, when Storyboard writes a shot where the character is seen from behind, that shot's `characters[].reference` automatically resolves to the locked back-angle image instead of the front reference — with zero effect on any existing project that never uses this feature at all. All newly locked primary and turnaround character references are isolated character-only assets on a plain neutral background with no scene or environment content — this is a stated success condition of the feature, not an implementation detail that can silently regress.
