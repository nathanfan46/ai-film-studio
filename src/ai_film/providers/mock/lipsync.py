...
class MockLipsyncProvider:
    def __init__(self, fail_first_n_submits: int = 0, polls_until_complete: int = 1):
        ...
        self._requests: dict[str, LipsyncGenerationRequest] = {}
    ...
    def submit(self, request: LipsyncGenerationRequest) -> GenerationJob:
        ...
        job_id = f"mock-lipsync-{self._submit_calls}"
        self._requests[job_id] = request
        self._poll_counts[job_id] = 0
        return GenerationJob(provider="mock", id=job_id, capability=Capability.LIPSYNC)
    ...
    def get_result(self, job: GenerationJob) -> VideoGenerationResult:
        ...
        output_path.write_bytes(b"MOCK-LIPSYNC-MP4-DATA")
