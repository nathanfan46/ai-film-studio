...
- Separately, check whether the project's `config.json` has a top-level `"template"` key. If it does, read `templates/<that value>/template.json` directly and read the result once now — its `shot_patterns` are optional grounding for pacing/structure inspiration during this brainstorm, the same way an approved reference-video brief is. If the key is absent, proceed with no template grounding — this is purely additive.
