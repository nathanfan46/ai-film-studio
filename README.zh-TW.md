...
（目前是 `h3-max` 與 `seedance-2.5`，已分別對照各自的 fal.ai 官方 OpenAPI schema
驗證過）真的會採用結尾畫面；
...
**`seedance-2.5`**（ByteDance，透過 fal.ai）單次呼叫最長可原生生成 30 秒，遠超過這裡
其他影片模型都受限的幾秒鐘長度。適合用在真正屬於同一個連續physical動作的橋段——例如
角色接起響鈴的電話——把它寫成一個較長的鏡頭，而不是拆成好幾個短鏡頭，可以避免一個
真實觀察到的失敗模式：每個獨立生成的鏡頭都會重新「發明」一次轉場動作（電話在鏡頭 A
結尾已經貼到耳邊，鏡頭 B 卻又從頭拿起一次），因為沒有任何機制能讓動作連戲跨越一個
人為安插的剪接點，不像單一連續生成那樣天生連貫。但它沒辦法解決同一動作的多機位覆蓋
問題（例如打鬥戲要在遠景和特寫之間剪接）——每次獨立的生成呼叫依然完全不知道其他呼叫
做了什麼，所以同一段動作要在剪接點對得上，還是得靠別的機制，不是單純把單一鏡頭拉長
就能解決。
